...
app.prepare().then(() => {
  ...
  io.on("connection", (rawSocket: Socket) => {
    ...
    socket.on("user:online", ({ userId, role }: { userId: string; role: Role }) => {
    ...
    socket.on(
      ...
      ({ dispatchId, status }: { dispatchId: string; status: AmbulanceStatus }) => {
    ...
    socket.on(
      "booking:created",
      ({ professionalId, booking }: { professionalId: string; booking: Booking }) => {
